...
DEFAULT_MODEL_PATH = "yoloe-11m-seg.pt"
CLASS_PROMPTS = ["products"]
WINDOW_NAME = "YOLOE Product"


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a YOLOE model on a video stream and display detections."
    )
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Path to the YOLOE weights file.",
    )
    parser.add_argument(
        "--confidence",
        type=float,
...
def run_stream(model_path: str, raw_source: str, confidence: float) -> None:
    ...
    text_pe = model.get_text_pe(CLASS_PROMPTS)
